...
export const JourneyToday = () => {
  ...
  const { data: recommendation, isLoading: recommendationLoading } = useQuery({ queryKey: ["daily-recommendation"], queryFn: () => fetchTodayRecommendation(user!.id), enabled: !!user });
  ...
  const generate = async () => {
    ...
    try {
      ...
      await client.invalidateQueries({ queryKey: ["daily-recommendation"] });
    } catch {
  ...
  const toggle = async (id: string, completed: boolean) => {
    ...
    try {
      ...
      await client.invalidateQueries({ queryKey: ["journey-task-completions"] });
    } catch {
      toast.error("That task couldn't be saved — please try again.");
    }
  };
  if (recommendationLoading) return <section className="paper-card p-5"><p className="text-sm text-muted-foreground">Preparing today’s rhythm…</p></section>;
