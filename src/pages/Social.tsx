...
export const ChallengeCommunity = () => {
  const { data: posts = [] } = useQuery({ queryKey: ["social-posts"], queryFn: async () => {
    const { data, error } = await supabase.from("challenge_submissions").select("id,caption,submitted_at,media_assets(path, bucket)").in("status", ["approved", "featured"]).order("submitted_at", { ascending: false });
